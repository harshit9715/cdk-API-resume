...
exports.handler = async (event, context) => {
  ...
  try {
    if (event.pathParameters){
      queryData["_id"] = ObjectId(event.pathParameters.id);
    }
  }
  catch (e) {
    return {
  ...
  const response = {
    ...
    body: JSON.stringify((event.pathParameters && event.pathParameters.id)? feedbacks.length > 0 ? feedbacks[0] : {} : feedbacks),
  };
