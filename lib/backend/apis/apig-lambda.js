...
class CdkBlog_Apig_Lambda_Stack extends cdk.NestedStack {
	...
	constructor(scope, id, props) {
		...
		const requestValidatorAll = new apigateway.RequestValidator(this, 'queryAndBodyValidator', {
			restApi,
			validateRequestParameters: true,
			validateRequestBody: true
		})

		const get_feedbacks = lambda_node14_integ(this, 'get-feedbacks', `${__dirname}/../functions/get-feedbacks/index.js`);
		const add_feedback = lambda_node14_integ(this, 'add-feedback', `${__dirname}/../functions/add-feedback/index.js`);
		const update_feedback = lambda_node14_integ(this, 'update-feedback', `${__dirname}/../functions/update-feedback/index.js`);
...
const lambda_node14_integ = (that, functionName, entry, handler = "handler") => {
